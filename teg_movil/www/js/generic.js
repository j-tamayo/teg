...
$(document).ready(function(){
    ...
    $('#registro_form').submit(function(event){
        ...
        $.post('http://192.168.1.141:8000'+url, data)
        .done(function(json){
    ...
    $('#login_form').submit(function(event){
        ...
        $.post('http://192.168.1.141:8000/api/login/', data)
        .done(function(json){
    ...
    $('#get_password_form').submit(function(event){
        ...
        $.post('http://192.168.1.141:8000/api/recuperar-clave/', data)
        .done(function(json){
    ...
    $('#reclamo_form').submit(function(event){
        ...
        $.post('http://192.168.1.141:8000/api/guardar-reclamo/', data)
        .done(function(json){
            console.log(json);
    ...
    $(document).on('click', '#next_request_page', function(){
        if(page_sol == 3 && $('#horario_sol').val().trim()){
            ...
            $.post('http://192.168.1.141:8000/api/crear-solicitud/', data)
            .done(function(json){
                console.log(json);
        ...
        if(page_sol >= 1 && page_sol < 3){
            if(page_sol == 1 && ($('#tipo_sol').val().trim() && $('#estado_sol').val().trim() && $('#municipio_sol').val().trim() && $('#fecha_asistencia_sol').val().trim())){
                ...
                page_sol++;
                $.post('http://192.168.1.141:8000/api/centros-sol/', {'municipio_id': $('#municipio_sol').val(), 'estado_id':$('#estado_sol').val()})
                .done(function(json){
                ...
                if(page_sol == 2 && $('#centros_inspeccion_sol').find('a').hasClass('ui-btn-active')){
                    $('#request_form_page' + page_sol).hide('fade');
                    page_sol++;
                    $.post('http://192.168.1.141:8000/api/horarios/', {'id_centro': $('#centro_id_sol').val(), 'fecha': $('#fecha_asistencia_sol').val(), 'id_tipo_inspeccion': $('#tipo_sol').val()})
                    .done(function(json){
    ...
    $(document).on('click', '.notificacion_item', function(){
        ...
        if(flag_leida == 'false'){
            $.post('http://192.168.1.141:8000/api/marcar-notificacion/', {'notificacion_usuario_id': notificacion_usuario_id, 'flag_marca': 1})
            .done(function(json){
                console.log(json);
    ...
    $(document).on('click', '.notificacion_item_elim', function(){
        ...
        $(document).on('click', '#noti_elim_confirm', function(event){
            ...
            $.post('http://192.168.1.141:8000/api/marcar-notificacion/', {'notificacion_usuario_id': notificacion_usuario_id, 'flag_marca': 2})
            .done(function(json){
                console.log(json);
                next_page = ref;
    ...
    $(document).on('click', '.solicitud_item_elim', function(){
        ...
        $(document).on('click', '#sol_elim_confirm', function(event){
            ...
            ref = $(this).attr('href');
            trans = $(this).attr('data-transition');
            $.post('http://192.168.1.141:8000/api/marcar-solicitud/', {'solicitud_id': solicitud_id})
            .done(function(json){
                console.log(json);
                next_page = ref;
                next_page_trans = trans;
